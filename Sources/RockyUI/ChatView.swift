...
struct ChatView: View {
    ...
    private func handleKey(_ event: NSEvent) -> NSEvent? {
        ...
        // An open menu, the settings, sheets (a permission request, an alert) and the slash command popup keep Esc for
        // closing themselves (CMD-03). The popup's state is read from the controller, a reference, at the key's time.
        if event.keyCode == 53, modifiers.isEmpty, chat.state == .running, menus?.open == nil,
           !SettingsPresenter.shared.isPresented, window.attachedSheet == nil, !composerText.popup.isOpen {
